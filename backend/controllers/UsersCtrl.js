...
const LoginUser=async(req,res)=>{
    try {
        ...
        if(!check){
            return res.json({status:false,msg:"Password doesn't match"})
        }
        const token = jwt.sign({user:{email:find.email,name:find.name,image:find.image,role:find.role},id:find._id},process.env.JWT_SECRET,{expiresIn:'1d'})
