import "./EastWeeklyCalendar.scss";
...
import { AddRowToEastCalendar, 
    DeleteRowEastCalendar, 
    LoadDispatchEastCalendar, 
    DeleteTrucksInfoEastCalendar,
    DeleteLoadInfoEastCalendar, 
    DragAndDropEastCalendar, 
    UpdateLoadEastCalendar,
    AddLoadInfoEastCalendar,
    UnDispatchEastCalendar,
    DeleteLoadAddInfoEastCalendar,
    DeleteCalendarTextEastCalendar
 } from "../../redux/actions/EastCalendarActions";
import { ToastHandler } from "../../redux/ToastHandler";
import { Button, Tooltip } from "@mui/material";
import { eastTextColumns } from '../../datatablesource';
import EastNoteDialog from "../dedicatedLaneDialog/EastNoteDialog";
import EastNoteAddDialog from "../dedicatedLaneDialog/EastNoteAddDialog";

const EastWeeklyCalendar = () => {
    ...
    const { weeklyCalendarInfos, calendarTexts } = useSelector((state) => state.EastCalendarReducer)
    ...
    const editText = (id) =>{
        dispatch({ type: "EDIT_EASTNOTEPAD_MODAL", payload: { open: true, id } });
    }

    const deleteText = (id) =>{
        const text = "Are you sure to delete it?";
        if(confirm(text)) {
            dispatch(DeleteCalendarTextEastCalendar(id, token));
        }
    }

    const addRow = (date) => {
        dispatch(AddRowToEastCalendar(date, token));
    }
    ...
    const loadDispatch = (id, rowInfo) => {
        ...
        if( tempTrucks?.length === 0 || !tempLoad  ) ToastHandler({status: null, msg: "Load or Truck isn't ready!"});
            ...
            dispatch(LoadDispatchEastCalendar(tempTrucks, tempLoad, id, token));
        }
    }
    ...
    const unDispatch = (id, rowInfo) => {
        ...
        if( tempTrucks?.length === 0 || !tempLoad  ) ToastHandler({status: null, msg: "Load or Truck isn't ready!"});
            ...
            dispatch(UnDispatchEastCalendar(tempTrucks, tempLoad, id, token));
        }
    }
    ...
    const deleteTrucksInfo = (data, trucks, id) => {
        ...
        if(confirm(text)) {
            dispatch(DeleteTrucksInfoEastCalendar(changedTrucks, data.value , id, token));
        }
    }
    ...
    const deleteLoadInfo = (id) => {
        const text = "Are you sure you want to delete it?";
        if(confirm(text)) {
            dispatch(DeleteLoadInfoEastCalendar(id, token));
        }
    }
    ...
    const deleteLoadAddInfo = (id) => {
        const text = "Are you sure to delete it?";
        if(confirm(text)) {
            dispatch(DeleteLoadAddInfoEastCalendar(id, token));
        }
    }
    ...
    const deleteRow = (id) => {
        const text = "Are you sure you want to delete it?";
        if(confirm(text)) {
            dispatch(DeleteRowEastCalendar(id, token));
        }
    }
    ...
    const drop = (ev, dropId) => {
        ...
        if (dragId !== "" && dragItem !== "" && dropId !== "" && dragId !== dropId) {
            ...
            dispatch(DragAndDropEastCalendar(dragTrucks, dragId, dropTrucks, dropId, dragItem.value, token));
        }
    };
            ...
            if(judgeWeek === "current") {
                ...
                if( judge < 0 ) {
                    if(loadInfo){
                        ...
                        dispatch(UpdateLoadEastCalendar(tempLoad, 'current', id, token));
                    } else dispatch(UpdateLoadEastCalendar(editedLoad, 'current', id, token));
                    setLoadInfo('');
                } else {
                    if(loadInfo || editedLoad.info) {
                        if(!loadInfo && !editedLoad.info)  ToastHandler({status: null, msg: 'The load already exist!'});
                            ...
                            if(!loadInfo && editedLoad.info) {
                                if(blank) {
                                    dispatch(AddLoadInfoEastCalendar(loadInfo, id, token));
                                    setLoadInfo('');
                                }
                ...
                if( judge < 0 ) {
                    if(loadInfo){
                        let tempLoad = editedLoad;
                        tempLoad.info = loadInfo;
                        dispatch(UpdateLoadEastCalendar(tempLoad, 'next', id, token));
                    } else dispatch(UpdateLoadEastCalendar(editedLoad, 'next', id, token));
                    setLoadInfo('');
                } else {
                    if(loadInfo || editedLoad.info) {
                        if(!loadInfo && !editedLoad.info)  ToastHandler({status: null, msg: 'The load already exist!'});
                                                        ...
                                                        <TableCell className="loadCell"  onDoubleClick={(e)=>loadEditHandle(item.load, item._id)}>
                                                            {item.editLoad && 
                                                                <>
                                                                    ...
                                                                    <button className="loadButton" onClick={(e)=>onChangeLoadHandle(item._id, "current")}>Add</button>
                                                                </>
                                                            }
                                                            {!item.editLoad &&
                                                                <>
                ...
                <div>
                    ...
                    <div className="calendarText">
                        ...
                        <DataGrid
                            ...
                            columns={eastTextColumns.concat(actionColumn)}
                            pageSize={10}
...
export default EastWeeklyCalendar;