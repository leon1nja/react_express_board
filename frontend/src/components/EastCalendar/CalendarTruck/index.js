...
import { UpdateTrucksEastCalendar, AddTruckInfoEastCalendar } from '../../../redux/actions/EastCalendarActions';
...
const CalendarTruck = (props) =>  {
    ...
    const { weeklyCalendarInfos } = useSelector(state => state.EastCalendarReducer);
    ...
    const setAddText = () => {
        if(truckInfo) {
            if(selectedChips.length > 0){
                ...
                dispatch(UpdateTrucksEastCalendar(tempChips, props.id, token));
                setAutoSelectFlag(false);
            } else {
                if(props.items[0]?.value) {
                    dispatch(AddTruckInfoEastCalendar(truckInfo, props.id, token));
                    setAutoSelectFlag(false);
                }
            ...
            if(props.items[0]?.info) {
                ...
                if(selectedChips.length > 0) {
                    dispatch(UpdateTrucksEastCalendar(selectedChips, props.id, token));
                    setAutoSelectFlag(false);
                } else {
