import "./MidwestWeeklyCalendar.scss";
...
import { AddRowToMidwestCalendar, 
    DeleteRowMidwestCalendar, 
    LoadDispatchMidwestCalendar, 
    DeleteTrucksInfoMidwestCalendar,
    DeleteLoadInfoMidwestCalendar, 
    DragAndDropMidwestCalendar, 
    UpdateLoadMidwestCalendar,
    AddLoadInfoMidwestCalendar,
    UnDispatchMidwestCalendar,
    DeleteLoadAddInfoMidwestCalendar,
    DeleteCalendarTextMidwestCalendar
 } from "../../redux/actions/MidwestCalendarActions";
import { ToastHandler } from "../../redux/ToastHandler";
import { Button, Tooltip } from "@mui/material";
import { midwestTextColumns } from '../../datatablesource';
import MidwestNoteDialog from "../dedicatedLaneDialog/MidwestNoteDialog";
import MidwestNoteAddDialog from "../dedicatedLaneDialog/MidwestNoteAddDialog";

const MidwestWeeklyCalendar = () => {
    ...
    const { weeklyCalendarInfos, calendarTexts } = useSelector((state) => state.MidwestCalendarReducer)
    ...
    const editText = (id) =>{
        dispatch({ type: "EDIT_MIDWESTNOTEPAD_MODAL", payload: { open: true, id } });
    }

    const deleteText = (id) =>{
        const text = "Are you sure to delete it?";
        if(confirm(text)) {
            dispatch(DeleteCalendarTextMidwestCalendar(id, token));
        }
    }

    const addRow = (date) => {
        dispatch(AddRowToMidwestCalendar(date, token));
    }
    ...
    const loadDispatch = (id, rowInfo) => {
        ...
        if( tempTrucks?.length === 0 || !tempLoad  ) ToastHandler({status: null, msg: "Load or Truck isn't ready!"});
            ...
            dispatch(LoadDispatchMidwestCalendar(tempTrucks, tempLoad, id, token));
        }
    }
    ...
    const unDispatch = (id, rowInfo) => {
        ...
        if( tempTrucks?.length === 0 || !tempLoad  ) ToastHandler({status: null, msg: "Load or Truck isn't ready!"});
            ...
            dispatch(UnDispatchMidwestCalendar(tempTrucks, tempLoad, id, token));
        }
    }
    ...
    const deleteTrucksInfo = (data, trucks, id) => {
        ...
        const text = "Are you sure to delete it?";
        if(confirm(text)) {
            dispatch(DeleteTrucksInfoMidwestCalendar(changedTrucks, data.value , id, token));
        }
    }
    ...
    const deleteLoadInfo = (id) => {
        const text = "Are you sure to delete it?";
        if(confirm(text)) {
            dispatch(DeleteLoadInfoMidwestCalendar(id, token));
        }
    }
    ...
    const deleteLoadAddInfo = (id) => {
        const text = "Are you sure to delete it?";
        if(confirm(text)) {
            dispatch(DeleteLoadAddInfoMidwestCalendar(id, token));
        }
    }
    ...
    const deleteRow = (id) => {
        const text = "Are you sure to delete it?";
        if(confirm(text)) {
            dispatch(DeleteRowMidwestCalendar(id, token));
        }
    }
    ...
    const drop = (ev, dropId) => {
        ...
        if (dragId !== "" && dragItem !== "" && dropId !== "" && dragId !== dropId) {
            ...
            dispatch(DragAndDropMidwestCalendar(dragTrucks, dragId, dropTrucks, dropId, dragItem.value, token));
        }
    };

    const onChangeLoadHandle = (id, judgeWeek) => {
        if(!editedLoad) ToastHandler({status: null, msg: 'Please complete load!'})
        else{
            if(judgeWeek === "current") {
                ...
                if( judge < 0 ) {
                    if(loadInfo){
                        ...
                        dispatch(UpdateLoadMidwestCalendar(tempLoad, 'current', id, token));
                    } else dispatch(UpdateLoadMidwestCalendar(editedLoad, 'current', id, token));
                    setLoadInfo('');
                } else {
                    if(loadInfo || editedLoad.info) {
                        if(!loadInfo && !editedLoad.info)  ToastHandler({status: null, msg: 'The load already exist!'});
                            ...
                            if(!loadInfo && editedLoad.info) {
                                if(blank) {
                                    dispatch(AddLoadInfoMidwestCalendar(loadInfo, id, token));
                                    setLoadInfo('');
                                }
                ...
                if( judge < 0 ) {
                    if(loadInfo){
                        let tempLoad = editedLoad;
                        tempLoad.info = loadInfo;
                        dispatch(UpdateLoadMidwestCalendar(tempLoad, 'next', id, token));
                    } else dispatch(UpdateLoadMidwestCalendar(editedLoad, 'next', id, token));
                    setLoadInfo('');
                } else {
                    if(loadInfo || editedLoad.info) {
                        if(!loadInfo && !editedLoad.info)  ToastHandler({status: null, msg: 'The load already exist!'});
                ...
                <div>
                                                ...
                                                return (
                                                    <TableRow key={key}>
                                                        ...
                                                        <TableCell className="loadCell"  onDoubleClick={(e)=>loadEditHandle(item.load, item._id)}>
                                                            {item.editLoad && 
                                                                <>
                                                                    <input placeholder="Input the load" className="loadInput" defaultValue={editedLoad?.value} onChange={(e)=>onInputLoadHandle(e.target.value)} />
                                                                    <input placeholder="Add info" className="loadInput" defaultValue={editedLoad?.info} onChange={(e)=>setLoadInfoHandle(e.target.value)} />
                    ...
                    <div className="calendarText">
                        ...
                        <DataGrid
                            ...
                            columns={midwestTextColumns.concat(actionColumn)}
                            pageSize={10}
...
export default MidwestWeeklyCalendar;