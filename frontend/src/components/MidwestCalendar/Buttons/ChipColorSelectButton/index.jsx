...
import { ChangeTruckColorMidwestCalendar, ChangeLoadColorMidwestCalendar } from '../../../../redux/actions/MidwestCalendarActions';
...
const ChipColorSelectButton = (props) => {
    ...
    const selectColor = (name, type) => {
        if(type === "trucks") {
            ...
            dispatch(ChangeTruckColorMidwestCalendar(trucks, selectedTruck.value, props.id, token));
            setPalette(false);
        }else{
            let changedLoad = props.load;
            changedLoad.status = name;
            dispatch(ChangeLoadColorMidwestCalendar(changedLoad, props.id, token));
            setPalette(false);
        }
    }
