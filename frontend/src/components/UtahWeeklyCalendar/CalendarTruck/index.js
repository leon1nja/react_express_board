...
import { UpdateTrucksUtahCalendar, AddTruckInfoUtahCalendar } from '../../../redux/actions/UtahCalendarActions';
...
const CalendarTruck = (props) =>  {
    ...
    const { weeklyCalendarInfos } = useSelector(state => state.UtahCalendarReducer);
    ...
    const setAddText = () => {
        if(truckInfo) {
            if(selectedChips.length > 0){
                ...
                dispatch(UpdateTrucksUtahCalendar(tempChips, props.id, token));
                setAutoSelectFlag(false);
            } else {
                if(props.items[0]?.value) {
                    dispatch(AddTruckInfoUtahCalendar(truckInfo, props.id, token));
                    setAutoSelectFlag(false);
                }
            ...
            if(props.items[0]?.info) {
                ...
                if(selectedChips.length > 0) {
                    dispatch(UpdateTrucksUtahCalendar(selectedChips, props.id, token));
                    setAutoSelectFlag(false);
                } else {
