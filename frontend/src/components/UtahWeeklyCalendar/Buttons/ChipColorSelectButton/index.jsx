...
import { ChangeTruckColorUtahCalendar, ChangeLoadColorUtahCalendar } from '../../../../redux/actions/UtahCalendarActions';
...
const ChipColorSelectButton = (props) => {
    ...
    const selectColor = (name, type) => {
        if(type === "trucks") {
            ...
            dispatch(ChangeTruckColorUtahCalendar(trucks, selectedTruck.value, props.id, token));
            setPalette(false);
        }else{
            let changedLoad = props.load;
            changedLoad.status = name;
            dispatch(ChangeLoadColorUtahCalendar(changedLoad, props.id, token));
            setPalette(false);
        }
    }
