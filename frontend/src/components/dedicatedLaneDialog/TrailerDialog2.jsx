...
import {  AddTIllinoisAction } from "../../redux/actions/DedicatedLaneActions";
...
export default function TrailerDialog2({ children }) {
    ...
    const {  ttrailersDialog } = useSelector(state => state.TrailerReducer)
    ...
    const handleClose = () => {
        dispatch({type:"TTRAILER_DIALOG",payload:false})
    };
   
    const OnSubmitHandler = async (e) => {
        e.preventDefault()
        dispatch(AddTTrailerAction(state, token))
        dispatch({type:"TTRAILER_DIALOG",payload:false})
