...
export default function IllusionDialog2({ children }) {
  ...
  const { tillinoisesDialog } = useSelector(
  ...
  const handleClose = () => {
    dispatch({ type: "TILLINOIS_DIALOG", payload: false });
  };

  const OnSubmitHandler = async (e) => {
    e.preventDefault();
    dispatch(AddTIllinoisAction(state, token));
    dispatch({ type: "TILLINOIS_DIALOG", payload: false });
