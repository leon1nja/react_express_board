...
import { EditCalendarTextUtahCalendar } from "../../redux/actions/UtahCalendarActions";
...
export default function UtahNoteDialog({ children }) {
    const [text, setText] = React.useState("")
    const { token } = useSelector(state => state.UserReg)
    const { EditNotepadModal, calendarText } = useSelector(state => state.UtahCalendarReducer)
    const dispatch = useDispatch();

    const handleClose = () => {
        dispatch({type:"EDIT_UTAHNOTEPAD_MODAL", payload: {open: false, id: ""}})
    };
    ...
    const OnSubmitHandler = async (e) => {
        ...
        tempText.text = text;
        dispatch(EditCalendarTextUtahCalendar(tempText, token));
        dispatch({type:"EDIT_UTAHNOTEPAD_MODAL", payload: {open: false, id: ""}});
        setText("");
    }
