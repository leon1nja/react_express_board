...
import { EditCalendarTextMidwestCalendar } from "../../redux/actions/MidwestCalendarActions";
...
export default function MidwestNoteDialog({ children }) {
    const [text, setText] = React.useState("")
    const { token } = useSelector(state => state.UserReg)
    const { EditNotepadModal, calendarText } = useSelector(state => state.MidwestCalendarReducer)
    const dispatch = useDispatch();

    const handleClose = () => {
        dispatch({type:"EDIT_MIDWESTNOTEPAD_MODAL", payload: {open: false, id: ""}})
    };
    ...
    const OnSubmitHandler = async (e) => {
        ...
        tempText.text = text;
        dispatch(EditCalendarTextMidwestCalendar(tempText, token));
        dispatch({type:"EDIT_MIDWESTNOTEPAD_MODAL", payload: {open: false, id: ""}});
        setText("");
    }
