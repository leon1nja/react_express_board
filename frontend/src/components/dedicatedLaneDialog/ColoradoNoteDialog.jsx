...
import { EditCalendarTextColoradoCalendar } from "../../redux/actions/ColoradoCalendarActions";
...
export default function ColoradoNoteDialog({ children }) {
    const [text, setText] = React.useState("")
    const { token } = useSelector(state => state.UserReg)
    const { EditNotepadModal, calendarText } = useSelector(state => state.ColoradoCalendarReducer)
    const dispatch = useDispatch();

    const handleClose = () => {
        dispatch({type:"EDIT_COLORADONOTEPAD_MODAL", payload: {open: false, id: ""}})
    };
    ...
    const OnSubmitHandler = async (e) => {
        ...
        tempText.text = text;
        dispatch(EditCalendarTextColoradoCalendar(tempText, token));
        dispatch({type:"EDIT_COLORADONOTEPAD_MODAL", payload: {open: false, id: ""}});
        setText("");
    }
