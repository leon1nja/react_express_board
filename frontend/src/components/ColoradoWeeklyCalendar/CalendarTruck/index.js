...
import { UpdateTrucksColoradoCalendar, AddTruckInfoColoradoCalendar } from '../../../redux/actions/ColoradoCalendarActions';
...
const CalendarTruck = (props) =>  {
    ...
    const { weeklyCalendarInfos } = useSelector(state => state.ColoradoCalendarReducer);
    ...
    const setAddText = () => {
        if(truckInfo) {
            if(selectedChips.length > 0){
                ...
                dispatch(UpdateTrucksColoradoCalendar(tempChips, props.id, token));
                setAutoSelectFlag(false);
            } else {
                if(props.items[0]?.value) {
                    dispatch(AddTruckInfoColoradoCalendar(truckInfo, props.id, token));
                    setAutoSelectFlag(false);
                }
            ...
            if(props.items[0]?.info) {
                ...
                if(selectedChips.length > 0) {
                    dispatch(UpdateTrucksColoradoCalendar(selectedChips, props.id, token));
                    setAutoSelectFlag(false);
                } else {
