...
import { ChangeTruckColorColoradoCalendar, ChangeLoadColorColoradoCalendar } from '../../../../redux/actions/ColoradoCalendarActions';
...
const ChipColorSelectButton = (props) => {
    ...
    const selectColor = (name, type) => {
        if(type === "trucks") {
            ...
            dispatch(ChangeTruckColorColoradoCalendar(trucks, selectedTruck.value, props.id, token));
            setPalette(false);
        }else{
            let changedLoad = props.load;
            changedLoad.status = name;
            dispatch(ChangeLoadColorColoradoCalendar(changedLoad, props.id, token));
            setPalette(false);
        }
    }
